...
class fcn_topdown(nn.Module):
    ...
    def forward(self, laterals, backbone_stage_size):
        ...
        # print(x6[0].size())  # [b, 150, 270, 480] - logit
        # print(x6[1].size())  # [b, 150, 270, 480] - softmax
        return x6
...
class fcn_topdown_predict(nn.Module):
    ...
    def forward(self, x):
        x = self.dropout(x)
        x = self.conv1(x)            # out_logit
        # ->
        x_softmax = self.softmax(x)  # out_softmax
        return x, x_softmax
