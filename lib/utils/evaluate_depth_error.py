...
'''evaluate'''
def evaluate_err(pred, gt, smoothed_criteria, mask=None, scale=10.0, range='out_range'):
    if type(pred).__module__ != np.__name__:
        pred = pred.cpu().numpy()
    if type(gt).__module__ != np.__name__:
        gt = gt.cpu().numpy()

    # print(pred.shape, gt.shape, mask.shape)  # (540, 960) (540, 960) (540, 960)
    pred = np.squeeze(pred)
    gt = np.squeeze(gt)
    # if mask is not None:
    #     gt = gt[mask[0]:mask[1], mask[2]:mask[3]]
